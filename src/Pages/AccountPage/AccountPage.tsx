...
export const AccountPage: React.FC<AccountPageProps> = ({ token }) => {
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      ...
      try {
        const response = await axios.get(`${config.api_path}/account`, config.headers());
  ...
  return (
    <>
    <Navbar isAuthenticated={!!token} onLogout={() => {
      localStorage.removeItem('token');
      window.location.href = '/'; // หรือใช้ navigate('/') ถ้ามี useNavigate()
    }} />
